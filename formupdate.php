<?php
    require("koneksi.php");
    $id = $_GET['id'];
    $sql = $pdo  -> prepare("SELECT * FROM regis WHERE id = '$id'");
    $sql -> execute();
...
<?php
    if (isset($_POST['nim'])) {
        ...
        $sql= $pdo -> prepare("UPDATE regis SET nama='$nama',nim='$nim',jenis_kelamin='$jk',fakultas='$fakultas',kelas='$kelas',hobi='$hobby',alamat='$alamat'");
        $sql -> execute();
        if($sql){
